...
class Level extends model {
    ...
    async createMaterial(inputs) {
        ...
        // console.log(result);
        await this.updateOrderNumber(afterLessonNumber, result.insertId, inputs)
        // return 'success';
    }
    async updateOrderNumber(afterLessonNumber, id, inputs) {
        // console.log(inputs.afterLessonNumber == "", "sajdkasjdksajdksa", afterLessonNumber == NaN);
        let queryLastLessonId = await this.select('levels', ['*']).where(['lesson_id=?']).values([inputs.lesson_id]).exec();
        this.dbConnection();
        ...
        if (inputs.afterLessonNumber == "") {
            queryOutput += `(${inputs.lesson_id},${id},${queryLastLessonId.length}) ON DUPLICATE KEY UPDATE order_number = VALUES(order_number)`;
        ...
        for (let i = parseInt(inputs.afterLessonNumber) - 1; i <= queryLastLessonId.length; i++) {
            if (i >= queryLastLessonId.length && id == queryLastLessonId[i - 1].id) {
                ...
                if (queryLastLessonId[i].order_number >= lessonNumber || queryLastLessonId[i].order_number <= lessonNumber) {
                    queryOutput += `(${inputs.lesson_id},${queryLastLessonId[i].id},${queryLastLessonId[i].order_number}),`
                } else {
    ...
    async createTask(inputs, content) {
        ...
        if (inputs.isTask == '1' || inputs.isTask != "" || inputs.title != "") {
            ...
            let afterLessonNumber = parseInt(inputs.afterLessonNumber) + 1;
            await this.updateOrderNumber(afterLessonNumber, result, inputs);
            return 'success';
        }
        return 'fail';
    }
